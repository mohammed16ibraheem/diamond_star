...
export default function Home() {
  return (
    <div className="min-h-screen bg-stone-100 dark:bg-stone-900 text-stone-900 dark:text-stone-100">
      <header className="border-b border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 px-4 py-3">
        <div className="max-w-5xl mx-auto">
          <h1 className="text-xl font-bold">Weighing System – Flow & Data</h1>
        </div>
      </header>

      <main className="max-w-5xl mx-auto px-4 py-8 space-y-12">
        {/* Diagram: what is happening – icons + animated flow */}
        <section className="weighing-section bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-6 shadow-sm">
            ...
            {FLOW_STEPS.map((item, i) => {
              ...
              return (
                <div key={item.step} className="flex items-center gap-1 sm:gap-2">
                  <div
                    className="weighing-step flex flex-col items-center min-w-[100px] sm:min-w-[130px] rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 p-4 text-center shadow-sm hover:shadow-md hover:scale-[1.02] transition-all duration-300"
                    style={{ animationDelay: `${i * 80}ms` }}
                  >
                    <span className="weighing-icon mb-2 inline-flex items-center justify-center w-10 h-10 rounded-full bg-amber-200/80 dark:bg-amber-800/50 text-amber-700 dark:text-amber-300">
                      <StepIcon className="w-5 h-5" strokeWidth={2.25} />
                    </span>
                    <span className="text-xs font-medium text-amber-700 dark:text-amber-400">Step {item.step}</span>
                    <span className="text-sm font-semibold mt-0.5">{item.title}</span>
                    <span className="text-xs text-stone-600 dark:text-stone-400 mt-1">{item.desc}</span>
                  </div>
                  {i < FLOW_STEPS.length - 1 && (
                    <span
                      className="weighing-arrow text-amber-500 dark:text-amber-400 self-center hidden sm:flex items-center"
        ...
        <section className="weighing-section bg-white dark:bg-stone-800 rounded-xl border border-stone-200 dark:border-stone-700 p-6 shadow-sm">
          <h2 className="text-lg font-semibold mb-2 flex items-center gap-2">
            <LayoutPanelTop className="w-5 h-5 text-amber-600 dark:text-amber-400" />
            Screens – where truck and data are filled
          </h2>
          <p className="text-stone-600 dark:text-stone-400 text-sm mb-6">
          ...
          <div className="grid gap-8 sm:grid-cols-1">
            {IMAGES.map((img, idx) => (
              <div key={idx} className="weighing-image-card space-y-2" style={{ animationDelay: `${idx * 100}ms` }}>
                <div className="relative rounded-lg overflow-hidden border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-stone-900 shadow-sm">
                  <Image
                    src={img.src}
                    alt={img.label}
                    width={800}
                ...
                <p className="text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 rounded px-2 py-1 inline-block">
                  {img.highlight}
                </p>
                {"dataFromScreen" in img && img.dataFromScreen && (
                  <div className="weighing-data-card mt-3 rounded-lg border border-stone-200 dark:border-stone-600 bg-stone-50 dark:bg-stone-900/80 p-4 text-sm">
                    <p className="font-semibold text-stone-800 dark:text-stone-200 mb-3">Data from this screen (when truck leaves)</p>
                    <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2 text-stone-600 dark:text-stone-400">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-stone-200 dark:border-stone-600">
                  ...
                  <th className="text-left py-2 pr-4 font-semibold">Where it is filled</th>
              ...
              <tbody>
                {DATA_FIELDS.map((row) => (
                  <tr key={row.field} className="weighing-table-row border-b border-stone-100 dark:border-stone-700/50">
                    ...
                    <td className="py-2 pr-4 text-stone-600 dark:text-stone-400">{row.where}</td>
                    <td className="py-2 text-stone-600 dark:text-stone-400">{row.meaning}</td>
                  </tr>
                ))}
              </tbody>
            </table>
